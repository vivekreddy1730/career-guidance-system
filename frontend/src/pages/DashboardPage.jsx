...
import { getReport, getRoadmap, getTrendingSkills } from "../api/endpoints";
  ...
  const card = (
    <div className="stat-card h-100">
      <div className="d-flex justify-content-between align-items-start mb-3">
        ...
        {link && (
          <Link to={link} className="text-brand" style={{ fontSize: "0.8rem" }}>
            View →
          </Link>
        )}
      </div>
      <div className="fw-800" style={{ fontSize: "1.5rem", color }}>
        {value}
      </div>
      <div className="fw-600 mb-1" style={{ fontSize: "0.875rem" }}>{label}</div>
      {sub && <div className="text-muted-dark" style={{ fontSize: "0.8rem" }}>{sub}</div>}
    </div>
  );
  return link ? <Link to={link} className="text-decoration-none">{card}</Link> : card;
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      getReport().then((r) => setReport(r.data.report)).catch(() => null),
      getRoadmap().then((r) => setRoadmap(r.data.roadmap)).catch(() => null),
      getTrendingSkills().then((r) => setTrendingSkills(r.data.trending_skills)).catch(() => []),
        ...
        <main className="page-content">
          ...
          <div className="row g-3 mb-4">
            ...
            <div className="col-6 col-lg-3">
              <StatCard
                ...
                sub={roadmap ? `${completedMilestones}/${totalMilestones} months done` : "Not generated"}
                color="#06b6d4"
                link="/roadmap"
              />
            </div>
          ...
          {roadmap ? (
            <div className="glass-card p-4 mb-4">
              ...
              <small className="text-muted-dark">{completedMilestones} of {totalMilestones} months completed · {roadmapProgress}%</small>
          ...
          <div className="row g-3">
            {[
              ...
              { icon: "💼", label: "Browse Jobs", sub: "Live listings", path: "/jobs", color: "#06b6d4" },
              ...
              <div key={i} className="col-6 col-md-3">
                <Link to={action.path} className="text-decoration-none">
                  <div className="stat-card text-center" style={{ cursor: "pointer" }}>
                    ...
                    <div className="fw-700" style={{ fontSize: "0.9rem" }}>{action.label}</div>
                    <div className="text-muted-dark" style={{ fontSize: "0.75rem" }}>{action.sub}</div>
                  </div>
                </Link>
              </div>
            ))}
